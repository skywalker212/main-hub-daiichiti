...
--                   Requests.NonCompleted[tonumber(reqcomp)]=nil
      ...
      {},Levels.Add,"<requestnumber> <type> <name>\t\t\t\tAdd release of given type. Enter the number of request that you are fulfilling wiyh this release, before category and release name (optional)."
    }
...
rightclick[{Levels.DelReq,"1 3",HubName.."\\Requests\\Delete a request","!"..Commands.DelReq.." %[line:ID number(s):]"}]=0
rightclick[{Levels.ShowReqs,"1 3",HubName.."\\Requests\\Show requests","!"..Commands.ShowReqs}]=0
...
function Connected (nick)
  if Requests.Completed[nick] then
    ...
    table.save(Requests.Completed, ScriptsPath.."data/requests_comp.dat")
    return "Your request (\""..reqdetails.."\") has been completed! It is named "..tune.." under category "..cat..". Has been added by "..goodguy,2
  end
end
...
function Start()
  ...
  for a,b in pairs(Types) do -- Add categories to rightclick. This MIGHT be possible on-the-fly, just get the DC ÜB3RH4XX0R!!!11one1~~~ guys to fucking document $UserCommand
    rightclick[{Levels.AddReq,"1 3",HubName.."\\Requests\\Add an item to the\\"..b,"!"..Commands.AddReq.." "..a.." %[line:Name:]"}]=0
    rightclick[{Levels.Add,"1 3",HubName.."\\Requests\\Fulfill an item in the\\"..b,"!"..Commands.Add.." %[line:Request number to be fulfilled (Enter if none):] "..a.." %[line:Name:]"}]=0
  end
end
